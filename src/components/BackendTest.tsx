...
export default function BackendTest() {
  ...
  const testResumeUpload = async () => {
    ...
    try {
      ...
      setResult(`✅ Resume upload successful: ${JSON.stringify(response, null, 2)}`);
    } catch (error: any) {
  ...
  const testLogin = async () => {
    ...
    try {
      ...
      setResult(`❌ Login failed (expected if no user exists): ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const checkBackendConnection = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3000/api/resume/Resume', {
        method: 'OPTIONS',
        mode: 'cors'
      });
      ...
      setResult(`❌ Cannot reach backend: ${error.message}. Make sure your backend is running on port 3000.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <div style={{ marginTop: '20px', fontSize: '14px', color: '#666' }}>
        ...
        <ol>
          <li>Make sure your backend server is running: <code>cd d:\ATSFRIENDLYRESUME_UI-main && node server.js</code></li>
